...
/* func TestCustomHeaderPolicyWithRetries(t *testing.T) {
	os.Setenv("AZURE_SDK_MAX_RETRIES", "1") //testing foe local env variable

	// Remember to clean up the environment variable after the test
	defer os.Unsetenv("AZURE_SDK_MAX_RETRIES")
	...
	// Simulate a pipeline transport that returns 429 until final retry, then 200

	mockTransport := transportFunc(func(req *http.Request) (*http.Response, error) {
		attempt++
		id := req.Header.Get("x-ms-client-request-id")

		// Verify request ID exists
		if id == "" {
			t.Errorf("Missing x-ms-client-request-id on attempt %d", attempt)
			return nil, nil
		}

		// Store first request ID and verify subsequent ones match
		if attempt == 1 {
			requestID = id
			t.Logf("Initial request ID: %s", requestID)
		} else if id != requestID {
			t.Errorf("Request ID changed: got %s, want %s (attempt %d)",
				id, requestID, attempt)
		}

		// Verify context contains same ID
		ctxID := req.Context().Value(clientRequestIDKey)
		if ctxID != id {
			t.Errorf("Context ID mismatch: header=%s, context=%v", id, ctxID)
		}

		// Return 429 to trigger retries
		if attempt <= maxRetries {
			return &http.Response{
				StatusCode: http.StatusTooManyRequests,
				Body:       io.NopCloser(strings.NewReader("Too many requests")),
	...
	// Perform the request through the pipeline
	resp, err := pl.Do(req)
	if err != nil {
	...
	// Ensure the request IDs are consistent across all retries
	expectedAttempts := maxRetries + 1
	if attempt != expectedAttempts {
		t.Errorf("Wrong number of attempts: got %d, want %d", attempt, expectedAttempts)
	}
